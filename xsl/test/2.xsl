...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="process-payer">
		...
		<xsl:key name="procedure-count" select="count(/remitt/procedure[payerkey=$payer and patientkey=$patient])" />
		<xsl:if test="count(/remitt/procedure[payerkey=$payer and patientkey=$patient]) &gt; 0">
			<showid><xsl:value-of select="$payer"/></showid>
			<showpat><xsl:value-of select="$patient"/></showpat>
		</xsl:if>
	</xsl:template>
