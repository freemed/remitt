...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="process-patient">
		...
		<xsl:for-each select="//provider">
			...
			<xsl:if test="count(//procedure[patientkey = $patient and providerkey=$prid]) &gt; 0">
				<!--
				<debug><xsl:text>pat,prov|</xsl:text>
	...
	<xsl:template name="process-provider">
		...
		<xsl:for-each select="//payer">
			...
			<xsl:if test="count(//procedure[patientkey = $patient and providerkey=$provider and payerkey=$payid]) &gt; 0">
				<!--
				<debug><xsl:text>patient key = </xsl:text>
					<xsl:value-of select="$patient"/>
					<xsl:text>, provider key = </xsl:text>
					<xsl:value-of select="$provider"/>
					<xsl:text>, payer key = </xsl:text>
	...
	<xsl:template name="process-payer">
		...
		<xsl:variable name="set" select="//procedure[patientkey = $patient and providerkey = $provider and payerkey = $payer]" />
		<debug><xsl:value-of select="count($set)"/><xsl:text> procedure(s) found</xsl:text></debug>
		<!--
			Now we use the craptastic method of producing a
			single form for a single procedure. What a crock.
		-->
			<!--
			<xsl:for-each select="$set">
			<debug><xsl:text>in loop|</xsl:text>
				<xsl:value-of select="$patient"/>
				<xsl:text>|</xsl:text>
				<xsl:value-of select="$provider"/>
				<xsl:text>|</xsl:text>
	...
	<xsl:template name="process-procedure-set">
		...
		<xsl:variable name="diagnosisset" select="set:distinct(exsl:node-set($set/diagnosis))" />
		<xsl:value-of select="$diagnosisset" />

		# of diags: <xsl:value-of select="count($diagnosisset)" />
		raw diags (distinct): <xsl:value-of select="$diagnosisset" />
		...
		<!--
		<debug><xsl:text>raw subset: </xsl:text><xsl:value-of select="$subset"/></debug>
		<debug><xsl:text>subset: </xsl:text><xsl:value-of select="set:intersection(/*/procedure, $subset)"/></debug>
		-->
		...
		<xsl:variable name="remaining" value="exsl:node-set(set:difference($set, $subset))" />
		<debug><xsl:text>count remaining: </xsl:text><xsl:value-of select="count(set:difference($set, $subset))"/></debug>

		<!-- if we have remaining items, time to recurse -->
		<!--
		<xsl:variable name="left" value="count($remaining) + 0" />
		<xsl:if test="$left &gt; 0">
			<debug><xsl:text>recursing (left &gt; 0)</xsl:text></debug>
			<xsl:call-template name="process-procedure-set">
				<xsl:with-param name="set" select="$remaining" />
			</xsl:call-template>
		</xsl:if>
		-->
	</xsl:template>
	...
	<xsl:template name="get-valid-diagnosis-set">
		...
		<xsl:variable name="diagnosisset" select="set:distinct(exsl:node-set($procedureset/diagnosiskey))" />
		<debug><xsl:text>debug diagset</xsl:text><xsl:copy-of select="$diagnosisset" /></debug>
		...
		<xsl:choose>
			<xsl:when test="count($diagnosisset) &gt; 4">
				...
				<xsl:call-template name="get-valid-diagnosis-set">
					<xsl:with-param name="procedureset" select="$procedureset[position() &lt;= count(exsl:node-set($procedureset))]" />
				</xsl:call-template>
	...
	<xsl:template name="generate-form">
		...
		<!--
		<xsl:variable name="procedure" select="//procedure[@id = $procedures]" />
		<debugprocedure><xsl:value-of select="$procedure/cpt4code"/></debugprocedure>
		<mydebug><xsl:value-of select="//payer[@id = //procedure[@id = $procedures]/payerkey]/name" /></mydebug>
		-->
	</xsl:template>
